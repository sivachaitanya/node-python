  ...
  // incoming request data can be accessed from req.body.XXXX and it has to be in JSON format
   ...
   try {
          ...
          pyshell.send({}).end(function (err) {
            if (err) {
              console.log(clc.red('****** Error/Warning from the Python shell - ',err))
              res.send({"error":err})
            };
            res.send({"result":out})
          }).on('message', function (data) {
            console.log(clc.green(' <<<<< stdout from py shell - ' + data))
             out[out.length] = data
           // res.send(data)
          }).on('stderr', function (data) {
            console.log(clc.red(' <<<<< stderr from py shell - ' + util.inspect(data)))
       ...
       console.log(clc.red('***** ERROR from python shell -  '+ util.inspect(error)))
       res.send({'error':error})
   }
